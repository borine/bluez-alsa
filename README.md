...
>
> 1. Option to limit A2DP SEPs to one per codec (from branch
>    `single-endpoint`). This may be needed for a (few, rare) devices which
>    fail when presented with more than one SEP per codec.
>
> 1. Option to disable A2DP codec reconfiguration (from branch
>    `a2dp-reconfigurable property`). Enforces the configuration selected by
>    default when the A2DP profile is connected.
>
--------------------------------------------------
