...
static bool hci_usb_interface_has_isoc_endpoint(const char *interface) {
	...
	while ((interface_ent = readdir(interface_dir))) {
		...
		const int iface_maxlen = PATH_MAX - sizeof(interface_ent->d_name) - sizeof("//type");
		snprintf(ep_path, PATH_MAX, "%.*s/%s", iface_maxlen, interface, interface_ent->d_name);
		DIR *ep_dir;
		...
		while ((ep_ent = readdir(ep_dir))) {
			...
			snprintf(file_path, PATH_MAX, "%.*s/%s/type", iface_maxlen, interface, interface_ent->d_name);
			FILE *file;
