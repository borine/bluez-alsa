...
unsigned int hci_sco_get_mtu(int sco_fd, struct ba_adapter *a) {
	 ...
	 * size, which is not considered by the Linux SCO socket driver when
	 * reporting the socket MTU. Using the wrong value can cause undefined
	 * behaviour, so we need get the correct value from the Linux USB
	 * subsystem. */
	if ((a->hci.type & 0x0F) == HCI_USB) {
		options.mtu = hci_usb_get_mtu(sco_fd, a);
