...
/**
 * Get read/write MTU for given SCO socket, as reported by the kernel bluetooth
 * socket API.
 *
 * @param sco_fd File descriptor of opened SCO socket.
 * @return On success this function returns MTU value. Otherwise, 0 is
...
unsigned int hci_sco_get_mtu(int sco_fd) {
	...
	if (poll(&pfd, 1, -1) == -1) {
		...
		if (getsockopt(sco_fd, SOL_SOCKET, SO_ERROR, &so_error, &len) != -1)
			if (so_error != 0)
				error_msg = strerror(so_error);
		error("SCO socket connect error: %s", error_msg);
		return 0;
	}
