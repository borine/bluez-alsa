...
repoll:
	...
	return io_bt_read(t, buffer->tail, ffb_blen_in(buffer));
}
...
static ssize_t a2dp_write_bt(struct io_thread_data *io, ffb_t *buffer) {
	...
	errno = 0;
	ret = io_bt_write(t, buffer->data, ffb_blen_out(buffer));
...
static bool a2dp_pcm_has_clients(struct ba_transport_pcm *pcm) {
	return pcm->multi == NULL ? pcm->fd != -1 : pcm->multi->client_count > 0;
}
...
static void *a2dp_sink_aptx_hd(struct ba_transport_thread *th) {
	...
	for (ba_transport_thread_ready(th);;) {
		...
		if (io_pcm_write(&t->a2dp.pcm, pcm.data, ffb_len_out(&pcm)) == -1)
			error("FIFO write error: %s", strerror(errno));
