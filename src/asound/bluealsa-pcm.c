...
static int bluealsa_hw_params(snd_pcm_ioplug_t *io, snd_pcm_hw_params_t *params) {
	...
	if (pcm->ba_pcm.channels != channels || pcm->ba_pcm.rate != rate) {
		if (pcm->ba_pcm.running) {
			snd_error(PCM, "Couldn't change BlueALSA PCM configuration");
			return -EINVAL;
		}
...
static int bluealsa_sw_params(snd_pcm_ioplug_t *io, snd_pcm_sw_params_t *params) {
	...
	if (silence_threshold != pcm->io_silence_threshold) {
		ba_debug_pcm(pcm, "Changing SW silence threshold: %zu -> %zu", pcm->io_silence_threshold, silence_threshold);
		pcm->io_silence_threshold = silence_threshold;
	}
	if (silence_size != pcm->io_silence_size) {
		ba_debug_pcm(pcm, "Changing SW silence size: %zu -> %zu", pcm->io_silence_size, silence_size);
	...
	if (stop_threshold != pcm->io_stop_threshold) {
		ba_debug_pcm(pcm, "Changing SW stop threshold: %zu -> %zu", pcm->io_stop_threshold, stop_threshold);
	...
	if (period_event != pcm->io_period_event) {
		ba_debug_pcm(pcm, "Changing SW period event: %d -> %d", pcm->io_period_event, period_event);
...
SND_PCM_PLUGIN_DEFINE_FUNC(bluealsa) {
	...
	if (codec_name[0] != '\0') {
		...
		if (pcm->ba_pcm.running) {
			...
			if (name_changed)
				snd_error(PCM, "Couldn't change BlueALSA PCM codec");
				...
				snd_error(PCM, "Couldn't change BlueALSA PCM codec configuration");
		}
		else {
