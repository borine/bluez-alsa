...
#define BLUEALSA_CLIENT_BUFFER_PERIODS (BLUEALSA_MULTI_CLIENT_THRESHOLD)

static bool ba_pcm_client_is_playback(struct bluealsa_pcm_client *client) {
	return client->multi->pcm->mode == BA_TRANSPORT_PCM_MODE_SINK;
}

static bool ba_pcm_client_is_capture(struct bluealsa_pcm_client *client) {
...
static size_t ba_pcm_client_playback_init_offset(const struct bluealsa_pcm_client *client) {
	const struct bluealsa_pcm_multi *multi = client->multi;
	const struct bluealsa_mix_buffer *buffer = &multi->playback_buffer;
	const size_t client_samples = client->in_offset * buffer->channels / buffer->frame_size;
	const size_t reported_delay = ba_pcm_mix_buffer_delay(buffer, buffer->end) + (BLUEALSA_MULTI_CLIENT_THRESHOLD * buffer->period);
...
static void ba_pcm_client_set_state(struct bluealsa_pcm_client *client, enum ba_pcm_client_state new_state) {
	...
	switch (new_state) {
		...
		case BLUEALSA_PCM_CLIENT_STATE_PAUSED:
			if (client->state == BLUEALSA_PCM_CLIENT_STATE_RUNNING && ba_pcm_client_is_capture(client))
				client->multi->active_count--;
			break;
...
static void ba_pcm_client_close_pcm(struct bluealsa_pcm_client *client) {
	if (client->pcm_fd != -1) {
...
static void ba_pcm_client_watch_drain(struct bluealsa_pcm_client *client, bool enabled) {
...
static ssize_t ba_pcm_client_read(struct bluealsa_pcm_client *client) {
	...
	do {
		...
		if ((ret = write(fd, buffer_, len)) == -1)
			...
			default:
				...
				ba_pcm_client_set_state(client, BLUEALSA_PCM_CLIENT_STATE_FINISHED);
				return;
			}
...
void ba_pcm_client_deliver(struct bluealsa_pcm_client *client) {
	...
	if (client->state == BLUEALSA_PCM_CLIENT_STATE_DRAINING1) {
		...
		if (client->in_offset == 0 && bytes == 0 && errno == EAGAIN) {
			size_t mix_avail = ba_pcm_mix_buffer_calc_avail(&multi->playback_buffer, multi->playback_buffer.mix_offset, client->out_offset);
			if (mix_avail == 0 || mix_avail > client->drain_avail) {
				...
				ba_pcm_client_set_state(client, BLUEALSA_PCM_CLIENT_STATE_DRAINING2);
...
static void ba_pcm_client_handle_playback_pcm(struct bluealsa_pcm_client *client) {
	...
	if (client->state == BLUEALSA_PCM_CLIENT_STATE_IDLE) {
		if (client->in_offset >= BLUEALSA_MULTI_CLIENT_THRESHOLD * client->multi->period_bytes)
			ba_pcm_client_set_state(client, BLUEALSA_PCM_CLIENT_STATE_RUNNING);
	}
}
...
static void ba_pcm_client_begin_drain(
	...
	if (ba_pcm_client_is_playback(client) && client->state == BLUEALSA_PCM_CLIENT_STATE_RUNNING) {
		ba_pcm_client_set_state(client, BLUEALSA_PCM_CLIENT_STATE_DRAINING1);
		ba_pcm_client_watch_pcm(client, false);
	}
	else {
...
static void ba_pcm_client_drop(struct bluealsa_pcm_client *client) {
...
static void ba_pcm_client_pause(struct bluealsa_pcm_client *client) {
	debug("PAUSE: client %zu", client->id);
	ba_pcm_client_set_state(client, BLUEALSA_PCM_CLIENT_STATE_PAUSED);
	ba_pcm_client_watch_pcm(client, false);
	if (ba_pcm_client_is_playback(client)) {
...
static void ba_pcm_client_resume(struct bluealsa_pcm_client *client) {
...
static void ba_pcm_client_handle_drain(struct bluealsa_pcm_client *client) {
...
static void ba_pcm_client_handle_control(struct bluealsa_pcm_client *client) {
	char command[6];
	ssize_t len;
	do {
...
struct bluealsa_pcm_client *ba_pcm_client_new(struct bluealsa_pcm_multi *multi, int pcm_fd, int control_fd) {
