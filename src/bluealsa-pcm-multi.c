...
bool bluealsa_pcm_multi_init(struct bluealsa_pcm_multi *multi, size_t transfer_samples) {
	...
	if (bluealsa_pcm_multi_is_playback(multi)) {
		...
		multi->delay = multi->period_frames * (BLUEALSA_MULTI_MIX_THRESHOLD + BLUEALSA_MULTI_CLIENT_THRESHOLD) * 10000 / multi->pcm->sampling;
...
ssize_t bluealsa_pcm_multi_read(struct bluealsa_pcm_multi *multi, void *buffer, size_t samples) {
	...
	{
		double scale_array[2] = { 1.0, 1.0 };
		if (multi->pcm->soft_volume) {
			scale_array[0] = multi->pcm->volume[0].scale;
			if (multi->pcm->channels == 2)
				scale_array[1] = multi->pcm->volume[1].scale;
		}
		else {
			if (multi->pcm->volume[0].scale == 0)
				scale_array[0] = 0;
			if (multi->pcm->volume[1].scale == 0)
				scale_array[1] = 0;
		}
