...
static int transport_acquire_bt_a2dp(struct ba_transport *t) {
	...
	debug("New A2DP transport: %d (MTU: R:%zu W:%zu)", fd, t->mtu_read, t->mtu_write);
...
fail:
	...
	if (rep != NULL)
		g_object_unref(rep);
	if (err != NULL) {
		error("Couldn't acquire A2DP transport: %s", err->message);
		g_error_free(err);
	}
...
fail:
	...
	if (rep != NULL)
		g_object_unref(rep);
	if (err != NULL) {
		error("Couldn't release A2DP transport: %s", err->message);
		g_error_free(err);
	}
...
static int transport_release_bt_sco(struct ba_transport *t) {
	...
	return shutdown(t->bt_fd, SHUT_RDWR);
}
...
int ba_transport_acquire(struct ba_transport *t) {
	...
	/* If releasing flag is set, it means that the BT socket might be
	 * closed at any moment. We should not return such BT socket file
	 * descriptor as a valid one. */
	if (t->bt_releasing) {
		errno = EBUSY;
		goto final;
	}

	/* If BT socket file descriptor is valid (and not in the releasing
	 * state), we can safely reuse it, e.g. in keep-alive mode. */
	if ((fd = t->bt_fd) != -1) {
		debug("Reusing BT socket: %d", t->bt_fd);
		goto final;
	}
...
int ba_transport_release(struct ba_transport *t) {
	...
	if (t->bt_fd == -1 || t->bt_releasing)
		goto final;

	if ((ret = t->release(t)) == 0)
		t->bt_releasing = true;
...
int ba_transport_close(struct ba_transport *t) {
	...
	debug("Closing BT socket: %d", t->bt_fd);

	close(t->bt_fd);
	t->bt_fd = -1;
	t->bt_releasing = false;
