...
bool io_worker_output_open_check(struct io_worker_output *output, size_t read_samples, size_t pcm_1s_samples) {
	if (output->pcm_open_retries > 0) {
		...
		if ((output->pcm_open_retry_pcm_samples += read_samples) <= interval * pcm_1s_samples)
			return false;
	}
	return true;
}

bool io_worker_output_open(struct io_worker_output *output,
					ffb_t *input_buffer) {
