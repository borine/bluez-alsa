...
struct resampler *resampler_new(
	...
	if ((resampler = calloc(1, sizeof(struct resampler))) == NULL) {
	...
	if ((resampler->src_state = src_new(type, channels, &err)) == NULL) {
		debug("Unable to create resampler: %s", src_strerror(err));
		goto fail;
	}

	if (in_format != SND_PCM_FORMAT_FLOAT) {
		if ((resampler->in_buffer = calloc(buffer_size, sizeof(float))) == NULL)
			goto fail;
	}

	if (out_format != SND_PCM_FORMAT_FLOAT) {
		if ((resampler->out_buffer = calloc(buffer_size, sizeof(float))) == NULL)
			goto fail;
	}
...
void resampler_destroy(struct resampler *resampler) {
	if (resampler == NULL)
		return;
...
int resampler_process(struct resampler *resampler, ffb_t *in, ffb_t *out) {
	...
	/* We must ensure that we only process as many samples
	 * as will fit into the output buffer. */
	const size_t out_samples = ffb_len_in(out);
	const size_t max_in_samples = out_samples / resampler->src_data.src_ratio;
	...
	if (resampler->in_format == SND_PCM_FORMAT_S16) {
		...
		/* Input is already in FLOAT format. */
	...
	while (true) {
		...
		if (resampler->out_format == SND_PCM_FORMAT_S16)
			src_float_to_short_array(resampler->out_buffer, out->tail,
					src_data->output_frames_gen * resampler->channels);
		else if (resampler->out_format == SND_PCM_FORMAT_S32)
			src_float_to_int_array(resampler->out_buffer, out->tail,
					src_data->output_frames_gen * resampler->channels);
