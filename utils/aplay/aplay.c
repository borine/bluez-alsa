...
static int io_worker_mixer_volume_sync_ba_pcm(
	...
	if (alsa_mixer_get_single_volume(&worker->alsa_mixer, vmax, &volume, &muted) != 0)
		return -1;
...
static int io_worker_mixer_volume_sync_alsa_mixer(
	...
	return alsa_mixer_set_single_volume(&worker->alsa_mixer, db, muted);
...
static void *io_worker_routine(struct io_worker *w) {
	...
	if (resampler_method != RESAMPLER_METHOD_NONE &&
			resampler_supports_format(pcm_format)) {
		use_resampler = true;
		pcm_flags = SND_PCM_NO_AUTO_RESAMPLE | SND_PCM_NO_AUTO_FORMAT;
		format_1 = SND_PCM_FORMAT_FLOAT_LE;
		format_2 = pcm_format;
	}
	else {
		format_1 = pcm_format;
		format_2 = SND_PCM_FORMAT_UNKNOWN;
	}
#endif
	...
	delay_report_init(&dr, &dbus_ctx, &w->ba_pcm, w->ba_pcm_fd, pcm_format_size * w->ba_pcm.channels);
	...
	for (;;) {
		...
		if (fds[1].revents & POLLIN) {
			...
			if (ffb_blen_in(&read_buffer) == 0) {
				...
				size_t discard_bytes = MIN(buffered, ffb_blen_out(&read_buffer));
				ffb_shift(&read_buffer, discard_bytes / pcm_format_size);
			}
			...
			case -EINVAL:
				...
				if (use_resampler && w->alsa_pcm.format == SND_PCM_FORMAT_UNKNOWN) {
					...
					if (alsa_pcm_open(&w->alsa_pcm, pcm_device, format_1,
								...
								SND_PCM_NO_AUTO_RESAMPLE, &tmp) == 0) {
						break;
					}
				}
#endif
		...
		if (alsa_pcm_write(&w->alsa_pcm, write_buffer,
					!w->ba_pcm.running, verbose >= 3) < 0)
			goto close_alsa;
		...
		if (!delay_report_update(&dr, &read_buffer, delay_frames, &err)) {
...
#if ENABLE_APLAY_RESAMPLER
		if (use_resampler && dr.values_i > ARRAYSIZE(dr.values)) {
...
static DBusHandlerResult dbus_signal_handler(DBusConnection *conn, DBusMessage *message, void *data) {
	...
	if (strcmp(interface, DBUS_INTERFACE_PROPERTIES) == 0) {
		...
		if ((worker = supervise_io_worker(pcm)) != NULL) {
			worker->ba_pcm.running = pcm->running;
