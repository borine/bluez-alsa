...
#if WITH_LIBSAMPLERATE
	pthread_cleanup_push(PTHREAD_CLEANUP(resampler_destroy), resampler);
	...
	for (;;) {
		...
		if (use_resampler) {
			...
			if (verbose >= 4 && rate_changed)
				debug("Resampler rate ratio changed: delay=%.1fms ratio=%.6f",
						1000.0 * dr.avg_value / w->ba_pcm.rate,
						resampler_current_rate_ratio(resampler));
		}
#endif
