...
bool io_worker_output_init(
					struct io_worker_output *output,
					snd_pcm_format_t input_format,
					unsigned int channels,
					unsigned int input_rate);

void io_worker_output_free(struct io_worker_output *output);

bool io_worker_output_open_check(
					struct io_worker_output *output,
					size_t read_samples,
...
void io_worker_output_close(struct io_worker_output *output);

int io_worker_output_write(struct io_worker_output *output,
					ffb_t *input_buffer,
					bool mute,
					bool drain);

snd_pcm_uframes_t io_worker_output_delay(
					const struct io_worker_output *output);

void io_worker_output_update_rate(struct io_worker_output *output,
					size_t frames_read, size_t delay);

static inline void io_worker_output_set_endianness(
					struct io_worker_output *output,
					void *buffer,
					size_t len,
...
static inline bool io_worker_output_is_open(const struct io_worker_output *output) {
	return alsa_pcm_is_open(&output->pcm);
}

static inline bool io_worker_output_is_running(const struct io_worker_output *output) {
