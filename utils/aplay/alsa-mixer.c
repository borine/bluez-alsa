...
int alsa_mixer_open(
	...
	mixer->has_db_scale = (snd_mixer_selem_get_playback_dB_range(mixer->elem,
					...
					mixer->volume_max_value > mixer->volume_min_value);
...
int alsa_mixer_get_volume(
	...
	if (mixer->has_db_scale) {
		...
		*volume = vmax * (volume_sum - mixer->volume_min_value) / (mixer->volume_max_value - mixer->volume_min_value);
	}
...
int alsa_mixer_set_volume(
	...
	if (mixer->has_db_scale) {
		...
		long value = mixer->volume_min_value + ((mixer->volume_max_value - mixer->volume_min_value) * volume / vmax);
